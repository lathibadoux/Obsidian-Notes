[[#006.3.1.1 Vector Databases]]
[[#006.3.1.2 Retrieval-Augmented Generation (RAG)]]
[[#006.3.1.3 Fine Tuning]]
## 006.3.1.1 Vector Databases
...
### RAG + Vector Search
RAG with Vector Search involves **retrieving** information using a vector database, **augmenting** the user’s prompt with that information and **generating** a response based on the user’s prompt and information retrieved using an LLM.

**Typical RAG Architecture (simplified)**
```sql
User Query
   ↓
Retriever → Vector Database (e.g., Pinecone, FAISS, Chroma)
   ↓
Relevant Context
   ↓
...
```pgsql
         ...
         ┌───────────────────────────────────────────────┐
         │               EMBEDDING MODEL                 │
         │  Converts text/query → numerical vector       │
         │                                               │
         │ 🔸 Examples:                                  │
         │   - OpenAI: text-embedding-3-small/large      │
         │   - HuggingFace: all-MiniLM, bge-base         │
         │   - Cohere: embed-english-v3.0                │
         │   - Google: textembedding-gecko               │
         │   - Voyage, Nomic, Instructor-xl              │
         └──────────────────┬────────────────────────────┘
                            │
                            ▼
         ┌───────────────────────────────────────────────┐
         ...
         │ Performs similarity search (cosine, dot, L2)  │
         │                                               │
         │ 🔸 Examples:                                  │
         │   - Local: Chroma, FAISS                      │
         │   - Distributed: Qdrant, Milvus, Weaviate     │
         │   - SQL-based: pgvector (Postgres), Redis     │
         │   - Managed: Pinecone, Elastic, Vespa.ai      │
         └──────────────────┬────────────────────────────┘
                            │
                            ▼
         ┌───────────────────────────────────────────────┐
         ...
         │ Optionally re-ranks or filters metadata       │
         │                                               │
         │ 🔸 Techniques:                                │
         │   - Similarity Search (cosine, dot product)   │
         │   - MMR (Max Marginal Relevance)              │
         │   - Multi-Query Expansion                     │
         │   - Rerankers (cross-encoders, Cohere Rerank) │
         └──────────────────┬────────────────────────────┘
                            │
                            ▼
         ┌───────────────────────────────────────────────┐
         │          LLM (GENERATOR / REASONER)           │
         │ Consumes retrieved context to compose answer  │
         │                                               │
         │ 🔸 Examples:                                  │
         │   - GPT-4, GPT-5 (OpenAI)                     │
         │   - Claude 3.x (Anthropic)                    │
         │   - Gemini (Google)                           │
         │   - Mistral, Llama-3, Command R+              │
         │                                               │
         ...
         │   Grounded, contextual, explainable output    │
         │   (can include citations & sources)           │
         └───────────────────────────────────────────────┘
