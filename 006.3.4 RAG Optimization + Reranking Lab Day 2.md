[[#006.3.4.1 classify_document_type]]
[[#006.3.4.2 determine_optimal_top_k]]
[[#006.3.4.3 rerank_results_gpt]]
[[#006.3.4.4 parsing-resume-with-matching-pubnub]]

Work on:
* Updating our Chat bot to use re-ranking and metadata filtering
* Create an asynchronous RAG email agent

* The optimal `top_k` value needs to be higher with re-ranking, which does significantly better with more vectors.  Must introduce noise into the `determine_optimal_top_k()` function in order to retrieve more `top_k` values.
...
* Where the metadata is filtered
...
# The prompt can be rewritten to not be so broad.  You can make it more selective so that it won't return so many results.
# This gives you power because your results can be more "noisy", but still use AI to step through them and have an accurate result.
# Essentially, we want to use re-ranking to cut down on the number of results.
...
                if item['similarity'] > .3: # This number is arbitrary and can cause the chatbot to not pull anything.  Remove this if the chatbot asks for more context in order to execute the query.
...
### 006.3.4.4 parsing-resume-with-matching-pubnub
* Copied the `parse-resume-with-matching` thread.  We are moving this thread to the PubNub worker.
...
        return {"parsed_resume": parsed_resume, "file_type": file_type}
...
# Want to create a dedicated job for our worker thread.
...
* When you are doing asynchronous stuff (ex. when we want the AI system to do more complicated work).

* PubNub gives you the ability to process data on the side.  It is a worker.  Remember, worker threads can take as long as they need to.  They don't have to be done in 30 seconds.

* PubNub will have two different queues:
	* `PUBNUB_JOB_CHANNEL` =  `pingpong_channel` (a worker is listening) 
	* `PUBNUB_RESPONSE_CHANNEL` = `pingpong_response_channel` (a worker is publishing)
	* Cannot have both channels named the same or PubNub will get stuck in an infinite loop

* python_pubnub_processor.py - This script listens on a PubNub queue for job requests, queries job context from Supabase, submits the request to OpenAI for processing, stores the response in Supabase, and publishes the response reference back to PubNub.

* Because the execution takes longer, we want to move the function `parse_resume_with_matching()` to the worker thread.
* To do this, we need to create another table in Supabase to read the PubNub.